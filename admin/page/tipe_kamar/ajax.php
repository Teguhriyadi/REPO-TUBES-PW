...
if ($request == 5) {
	...
	$sql = $con->query("SELECT * FROM tipe_kamar WHERE id_tipe = $id_tipe ");
	$data = $sql->fetch_assoc();
	$imageGambar = $data['image'];
	...
	if ($_FILES['image']['error'] === 4) {
		...
		if ($imageGambar != NULL) {
			if (file_exists("../../image/".$image_lama)) {
	...
	if (!empty($tmpname)) {
		...
		$query = $con->query("UPDATE tipe_kamar SET tipe_kamar = '$tipe_kamar', deskripsi = '$deskripsi', fasilitas = '$fasilitas' , harga = '$harga' , jumlah_bed = '$jumlah_bed' , image = '$namafilebaru' WHERE id_tipe = '$id_tipe' ");
	} else {
		$response = 0;

		$query = $con->query("UPDATE tipe_kamar SET tipe_kamar = '$tipe_kamar', deskripsi = '$deskripsi', fasilitas = '$fasilitas' , harga = '$harga' , jumlah_bed = '$jumlah_bed' WHERE id_tipe = '$id_tipe' ");
	}
